...
static void init_hw(void){
    ...
    while (sensor_count == 0){
        sensor_count = ds18x20_scan_devices((gpio_num_t)DSB_PIN, addrs, MAX_SENSORS, &found);
...
float get_ext_temp(void){
    ...
    for(int i=0; i < 10; ++i){
        ds18x20_measure_and_read_multi((gpio_num_t)DSB_PIN, addrs, sensor_count, temps);
